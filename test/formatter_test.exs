defmodule FormatterTest do
  ...
  test "iso8601 formatting" do
    ...
    assert Formatter.iso8601(time) == "2014-09-26T17:10:20-03:00"
    {:ok, time} = Kalends.DateTime.from_erl({{2014, 9, 26}, {7, 0, 2}}, "Europe/Copenhagen")
    assert Formatter.iso8601(time) == "2014-09-26T07:00:02+02:00"
    {:ok, time} = Kalends.DateTime.from_erl({{10, 9, 26}, {7, 0, 2}}, "Europe/Copenhagen")
    assert Formatter.iso8601(time) == "0010-09-26T07:00:02+00:50"
  end
