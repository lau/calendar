defmodule Kalends.NaiveDateTime do
  ...
  """
  defstruct [:year, :month, :day, :hour, :min, :sec, :microsec]
  ...
  """
  def from_erl!(erl_date_time, microsec \\ nil) do
    {:ok, result} = from_erl(erl_date_time, microsec)
    result
  end
  ...
  ## Examples
      ...
      {:ok, %Kalends.NaiveDateTime{day: 26, hour: 17, min: 10, month: 9, sec: 20, year: 2014, microsec: 321321} }

      iex>from_erl({{2014, 99, 99}, {17, 10, 20}})
      {:error, :invalid_datetime}
  """
  def from_erl({{year, month, day}, {hour, min, sec}}, microsec \\ nil) do
    if validate_erl_datetime {{year, month, day}, {hour, min, sec}} do
      {:ok, %Kalends.NaiveDateTime{year: year, month: month, day: day, hour: hour, min: min, sec: sec, microsec: microsec}}
    else
      {:error, :invalid_datetime}
    end
  end
  ...
  Takes a NaiveDateTime. Returns a DateTime.
      ...
      %Kalends.DateTime{abbr: "UTC", day: 15, microsec: nil, hour: 2, min: 37, month: 10, sec: 22, std_off: 0, timezone: "UTC", utc_off: 0, year: 2014}
  """
